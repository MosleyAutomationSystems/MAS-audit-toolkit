...
class SettingsDialog(tk.Toplevel):
    ...
    def _init_vars(self):
        ...
        self.var_large_targets = tk.BooleanVar(value=s.get("large_targets", False))
        ...
        for var in [self.var_theme, self.var_font_family, self.var_font_size,
                    ...
                    self.var_saturation, self.var_hue, self.var_large_targets]:
    ...
    def _read_vars(self):
        ...
        self.work.update({
            ...
            "large_targets":   self.var_large_targets.get(),
        })
    ...
    def _build_appearance_tab(self, nb, t: dict):
        ...
        # Large Targets toggle (C12 — Mosley Standard Category 5)
        tk.Label(f, text="Large targets:", bg=t["BG"], fg=t["FG"],
                 font=("Segoe UI", 10)).grid(
                     row=3, column=0, sticky="w", padx=16, pady=8)
        lt_frame = tk.Frame(f, bg=t["BG"])
        lt_frame.grid(row=3, column=1, sticky="w", padx=8, pady=8)
        tk.Checkbutton(
            lt_frame,
            text="Scale all interactive elements to 48px minimum height",
            variable=self.var_large_targets,
            bg=t["BG"], fg=t["FG"],
        ...
        tk.Label(
            lt_frame,
            text="Recommended for users with tremor or reduced motor precision.",
    ...
    def _reset_all(self):
        ...
        self.var_large_targets.set(d.get("large_targets", False))
...
class AuditApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # C12 — Large Targets: 48px min height when enabled, standard 6px otherwise.
        lt  = self.settings.get("large_targets", False)
        large_min = config.LARGE_TOUCH_TARGET   #48px
        btn_ipady = 10 if lt else 4
        inp_ipady = 10 if lt else 6
        self.configure(bg=t["BG"])
        ...
        self.source_entry.grid(row=2, column=0, columnspan=2,
                               sticky="ew", pady=(4, 0), ipady=inp_ipady)
        ...
        self.browse_btn.grid(row=2, column=2,
                             sticky="ew", padx=(8, 0), pady=(4, 0), ipady=inp_ipady)
        ...
        self.run_btn.grid(row=3, column=0, columnspan=3,
                          sticky="ew", pady=(10, 0), ipady=btn_ipady + 4)
        ...
        try:
            ...
            if not all_findings:
                ...
                severity_labels = {
                    ...
                    "medium":   "MEDIUM — Moderate Usability Issues",
                    "low":      "LOW — Quality Improvements",
                    "info":     "INFO — Informational",
                }
                severity_descriptions = {
                    "critical": "These issues prevent access for users with disabilities.",
                    "high":     "These issues significantly affect usability.",
                    "medium":   "These issues affect some users.",
                    "low":      "These issues are best-practice improvements.",
                    "info":     "These items are noted for awareness and do not require immediate action.",
                }
                ...
                for level in severity_order:
                    ...
                    for f in level_findings:
                        self._write_result(
                            ...
                            f"— WCAG {f['wcag']} (Level {f.get('level', 'MS')})\n",
                            tag="heading")
                        self._write_result(
                            f"       {f['message']}\n\n",
                            tag="error")
